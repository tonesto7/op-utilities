...
if [ "$OS" == "Darwin" ]; then
  ...
  BUILD_DIR=~/Documents/git-test/bp-public-exp-build
  TMP_DIR=~/Documents/git-test/bp-public-exp-build-tmp
else
...
DEV_BRANCH="bp-public-experimental"
GIT_REPO_ORIGIN="git@github.com:ford-op/sp-dev-c3.git"
PUBLIC_BRANCH="staging-DONOTUSE"
GIT_PUBLIC_REPO_ORIGIN="git@github.com:BluePilotDev/bluepilot.git"

# Remove existing BUILD_DIR and clone BluePilotDev/bluepilot repo
echo "[-] Removing existing BUILD_DIR and cloning BluePilotDev/bluepilot repo"
rm -rf $BUILD_DIR
git clone --single-branch --branch $PUBLIC_BRANCH $GIT_PUBLIC_REPO_ORIGIN $BUILD_DIR

# Change to the BUILD_DIR
cd $BUILD_DIR

# Remove all files from the cloned repo except .git
...
echo "[-] Committing removal of files"
git commit -m "Remove all files for clean slate"

# Clone the ford-op/sp-dev-c3 repo to the TMP_DIR
echo "[-] Cloning ford-op/sp-dev-c3 repo to TMP_DIR"
git clone --recurse-submodules --depth 1 $GIT_REPO_ORIGIN -b $DEV_BRANCH $TMP_DIR

# Copy all files from TMP_DIR to BUILD_DIR
...
git add --all
git commit --amend -m "bluepilot experimental | v$VERSION-$TIME_CODE
version: bluepilot experimental v$SP_VERSION release
date: $DATETIME
...
if [ ! -z "$PUSH" ]; then
  echo "[-] Pushing to $PUBLIC_BRANCH branch of public repo T=$SECONDS"
  git push -f origin $PUBLIC_BRANCH
fi
