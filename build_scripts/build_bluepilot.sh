...
"
  ...
  # check if $BUILD_BRANCH is available and delete
  if git show-ref --verify --quiet "refs/heads/$BUILD_BRANCH"; then
    echo "[-] Deleting Local $BUILD_BRANCH branch T=$SECONDS"
    git branch -D $BUILD_BRANCH
  fi

  # Check if remote branch exists and delete
  if git ls-remote --heads $ORIGIN_REPO $BUILD_BRANCH; then
    echo "[-] Deleting Remote $BUILD_BRANCH branch T=$SECONDS"
    git push $ORIGIN_REPO --delete $BUILD_BRANCH
  fi

  # Rename the temp branch to the desired branch
  echo "[-] Renaming temp_branch to $BUILD_BRANCH T=$SECONDS"
  git branch -m $BUILD_BRANCH

  # Force push to overwrite the remote branch
  echo "[-] Pushing to $BUILD_BRANCH branch of remote repo $ORIGIN_REPO T=$SECONDS"
